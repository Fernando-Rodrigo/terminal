    ...
    _currentLegacyColorAttribute(DEFAULT_COLOR_ATTRIBUTE)
{
    ...
    _fontSize.X = FontWidth > SHORT_MAX ? SHORT_MAX : (til::CoordType)FontWidth;
    _fontSize.Y = FontHeight > SHORT_MAX ? SHORT_MAX : (til::CoordType)FontHeight;
}
...
[[nodiscard]] HRESULT BgfxEngine::EndPaint() noexcept
{
    ...
    {
        for (til::CoordType i = 0; i < _displayHeight; i++)
        {
...
[[nodiscard]] HRESULT BgfxEngine::PaintBackground() noexcept
{
    ...
    for (til::CoordType i = 0; i < _displayHeight; i++)
    {
        ...
        for (til::CoordType j = 0; j < _displayWidth; j++)
        {
