...
// Return Value:
// - HRESULT S_OK, GDI-based error code, or safemath error
HRESULT GdiEngine::InvalidateScroll(const til::point pcoordDelta) noexcept
{
    if (pcoordDelta.X != 0 || pcoordDelta.Y != 0)
    {
        ...
        til::size szInvalidScrollNew;
        szInvalidScrollNew.cx = _szInvalidScroll.cx + ptDelta.x;
        szInvalidScrollNew.cy = _szInvalidScroll.cy + ptDelta.y;
...
// Return Value:
// - S_OK, GDI related failure, or safemath failure.
HRESULT GdiEngine::_InvalidCombine(const til::rect& prc) noexcept
{
...
// Return Value:
// - S_OK, GDI related failure, or safemath failure.
HRESULT GdiEngine::_InvalidateRect(const til::rect& prc) noexcept
{
