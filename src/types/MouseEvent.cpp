...
INPUT_RECORD MouseEvent::ToInputRecord() const noexcept
{
    ...
    record.Event.MouseEvent.dwMousePosition.X = gsl::narrow_cast<short>(std::clamp(_position.x, 0, SHRT_MAX));
    record.Event.MouseEvent.dwMousePosition.Y = gsl::narrow_cast<short>(std::clamp(_position.y, 0, SHRT_MAX));
