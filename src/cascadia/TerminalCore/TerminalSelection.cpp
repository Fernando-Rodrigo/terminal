...
void Terminal::_MoveByChar(SelectionDirection direction, til::point& pos)
{
    ...
    case SelectionDirection::Left:
        ...
        pos = _activeBuffer().GetGlyphStart(til::point{ pos });
        break;
    case SelectionDirection::Right:
        _activeBuffer().GetSize().IncrementInBounds(pos);
        pos = _activeBuffer().GetGlyphEnd(til::point{ pos });
        break;
    case SelectionDirection::Up:
    {
        const auto bufferSize{ _activeBuffer().GetSize() };
        pos = { pos.X, std::clamp(base::ClampSub<til::CoordType, til::CoordType>(pos.Y, 1).RawValue(), bufferSize.Top(), bufferSize.BottomInclusive()) };
        break;
    }
    case SelectionDirection::Down:
    {
        const auto bufferSize{ _activeBuffer().GetSize() };
        pos = { pos.X, std::clamp(base::ClampAdd<til::CoordType, til::CoordType>(pos.Y, 1).RawValue(), bufferSize.Top(), bufferSize.BottomInclusive()) };
        break;
    }
    }
}
...
void Terminal::_MoveByViewport(SelectionDirection direction, til::point& pos)
{
    ...
    case SelectionDirection::Up:
    {
        ...
        const auto newY{ base::ClampSub<til::CoordType, til::CoordType>(pos.Y, viewportHeight) };
    ...
    case SelectionDirection::Down:
    {
        ...
        const auto newY{ base::ClampAdd<til::CoordType, til::CoordType>(pos.Y, viewportHeight) };
