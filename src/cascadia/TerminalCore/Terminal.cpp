...
[[nodiscard]] HRESULT Terminal::UserResize(const til::size viewportSize) noexcept
{
    ...
    const auto newBufferHeight = ::base::ClampAdd(viewportSize.Y, _scrollbackLines);
    ...
    try
    {
        ...
        const std::optional<til::CoordType> oldViewStart{ oldViewportTop };
...
// Return value:
// - true, if we are tracking mouse input. False, otherwise
bool Terminal::ShouldSendAlternateScroll(const uint32_t uiButton,
                                         const int32_t delta) const noexcept
{
    return _terminalInput->ShouldSendAlternateScroll(uiButton, delta);
}
...
// - true if we translated the key event, and it should not be processed any further.
// - false if we did not translate the key, and it should be processed into a character.
bool Terminal::SendMouseEvent(til::point viewportPos, const uint32_t uiButton, const ControlKeyStates states, const int32_t wheelDelta, const TerminalInput::MouseButtonState state)
{
