...
LRESULT IslandWindow::_OnSizing(const WPARAM wParam, const LPARAM lParam)
{
    ...
    auto winRect = reinterpret_cast<RECT*>(lParam);
...
//   being moved at all.
// Arguments:
// - lParam: a til::rect& with the proposed window position, that should be filled
...
[[nodiscard]] LRESULT IslandWindow::MessageHandler(UINT const message, WPARAM const wparam, LPARAM const lparam) noexcept
{
    ...
    case WM_MOUSEWHEEL:
        try
        {
            ...
            const auto wheelDelta = static_cast<til::CoordType>(HIWORD(wparam));
