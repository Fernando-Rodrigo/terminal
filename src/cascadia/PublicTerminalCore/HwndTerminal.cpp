...
HRESULT HwndTerminal::Refresh(const til::size windowSize, _Out_ til::point* dimensions)
{
...
HRESULT _stdcall TerminalTriggerResize(_In_ void* terminal, _In_ til::CoordType width, _In_ til::CoordType height, _Out_ til::point* dimensions)
{
    const auto publicTerminal = static_cast<HwndTerminal*>(terminal);
...
/// <returns>HRESULT of the attempted resize.</returns>
HRESULT _stdcall TerminalTriggerResizeWithDimension(_In_ void* terminal, _In_ til::point dimensionsInCharacters, _Out_ til::size* dimensionsInPixels)
{
...
HRESULT _stdcall TerminalCalculateResize(_In_ void* terminal, _In_ til::CoordType width, _In_ til::CoordType height, _Out_ til::point* dimensions)
{
    const auto publicTerminal = static_cast<const HwndTerminal*>(terminal);
...
try
{
    ...
    if (uMsg == WM_MOUSEWHEEL || uMsg == WM_MOUSEHWHEEL)
    {
        ...
        auto coordsToTransform = cursorPosition;
        ScreenToClient(_hwnd.get(), coordsToTransform.as_win32_point());
